...
namespace CoreUI\Utils\Session;
 ...
 * @see Session
 */
require_once 'SessionAbstract.php';
...
class SessionNamespace extends SessionAbstract implements \IteratorAggregate
{
    ...
    public function __construct($namespace = 'Default', $singleInstance = false)
    {
        ...
        if (isset($_SESSION['__DATA__'][$namespace])) {

            // Expire Namespace by Namespace Hop (ENNH)
            if (isset($_SESSION['__DATA__'][$namespace]['ENNH'])) {
                $_SESSION['__DATA__'][$namespace]['ENNH']--;

                if ($_SESSION['__DATA__'][$namespace]['ENNH'] === 0) {
                    if (isset($_SESSION[$namespace])) {
                        self::$_expiringData[$namespace] = $_SESSION[$namespace];
                        unset($_SESSION[$namespace]);
                    }
                    unset($_SESSION['__DATA__'][$namespace]);
                }
            }

            // Expire Namespace Variables by Namespace Hop (ENVNH)
            if (isset($_SESSION['__DATA__'][$namespace]['ENVNH'])) {
                foreach ($_SESSION['__DATA__'][$namespace]['ENVNH'] as $variable => $hops) {
                    $_SESSION['__DATA__'][$namespace]['ENVNH'][$variable]--;

                    if ($_SESSION['__DATA__'][$namespace]['ENVNH'][$variable] === 0) {
                        ...
                        unset($_SESSION['__DATA__'][$namespace]['ENVNH'][$variable]);
                    }
                }
                if(empty($_SESSION['__DATA__'][$namespace]['ENVNH'])) {
                    unset($_SESSION['__DATA__'][$namespace]['ENVNH']);
                }
            }
        }

        if (empty($_SESSION['__DATA__'][$namespace])) {
            unset($_SESSION['__DATA__'][$namespace]);
        }

        if (empty($_SESSION['__DATA__'])) {
    ...
    public function setExpirationSeconds($seconds, $variables = null)
    {
        ...
        if ($variables === null) {
            ...
            $_SESSION['__DATA__'][$this->_namespace]['ENT'] = time() + $seconds;
            ...
            foreach ($variables as $variable) {
                if (!empty($variable)) {
                    $_SESSION['__DATA__'][$this->_namespace]['ENVT'][$variable] = time() + $seconds;
                }
            }
        }
    }
    ...
    public function setExpirationHops($hops, $variables = null, $hopCountOnUsageOnly = false)
    {
        ...
        if ($variables === null) {
            ...
            if ($hopCountOnUsageOnly === false) {
                $_SESSION['__DATA__'][$this->_namespace]['ENGH'] = $hops;
            } else {
                $_SESSION['__DATA__'][$this->_namespace]['ENNH'] = $hops;
            }
            ...
            foreach ($variables as $variable) {
                if (!empty($variable)) {
                    if ($hopCountOnUsageOnly === false) {
                        $_SESSION['__DATA__'][$this->_namespace]['ENVGH'][$variable] = $hops;
                    } else {
                        $_SESSION['__DATA__'][$this->_namespace]['ENVNH'][$variable] = $hops;
                    }
                }
            }
        }
